                ...
                <div class="ibox">
                    <div class="ibox-title">
                        ...
                        <div class="ibox-tools">
                            <a href="<?php echo U('Tag/tag_add');?>" class="btn btn-primary btn-xs">添加新标签</a>
                        </div>
                    </div>
                        ...
                        <div class="project-list">
                            <table class="table table-hover" style="text-align:center">
                                <tbody>
                                  <tr>
                                      <th>编号</th>
                                      <th><i class="fa fa-tag"></i>标签名称</th>
                                      <th><i class="fa fa-unlink"></i>标签地址</th>
                                      <th><i class="fa fa-adjust"></i>标签排序</th>
                                      <th><i class="fa fa-hand-o-down"></i>操作</th>
                                  </tr>
                                  <?php if(is_array($tag)): foreach($tag as $key=>$v): ?><tr>
                                        <td><?php echo ($a++); ?></td>
                                        <td><?php echo ($v["tag_name"]); ?></td>
                                        <td><?php echo ($v["tag_url"]); ?></td>
                                        <td><?php echo ($v["tag_ord"]); ?></td>
                                        <td>
                                            <a href="<?php echo U('Admin/Tag/tag_edit/tag_id/'.$v['tag_id']);?>" class="btn btn-white btn-sm"><i class="fa fa-pencil"></i> 编辑 </a>
                                            <a href="javascript:void(0)" class="btn btn-white btn-sm del" del_id="<?php echo ($v["tag_id"]); ?>"><i class="fa fa-trash"></i> 删除 </a>
                                        </td>
            ...
            var del_id = $(this).attr('del_id');
              swal({
                  ...
                  if (isConfirm) {
                      $.ajax({
                        type:'get',
                        url:"<?php echo U('Admin/Tag/tag_del');?>",
                        data:'del_id='+del_id,
                        success:function (z) {
                            console.log(z);
                            if(z==1) {
